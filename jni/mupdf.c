...
long lmemdoc = 0;
unsigned char *memdoc = NULL;

JNIEXPORT int JNICALL
Java_no_digipost_android_pdf_PDFCore_openFile(JNIEnv * env, jobject thiz, jstring jfilename)
{
	const char *filename;
	int pages = 0;
	int result = 0;
...
JNIEXPORT int JNICALL
Java_no_digipost_android_pdf_PDFCore_openMemmory(JNIEnv * env, jobject thiz, jbyteArray jbuffer ,int type)
{
	/* type C: cbz X: xps P: pdf*/
	char *buffer;
	int pages = 0;
	int result = 0;
	fz_stream *ffile;
	...
	lmemdoc = (*env)->GetArrayLength(env, jbuffer);
	buffer = (*env)->GetByteArrayElements(env,jbuffer,0);
	if (buffer == NULL || !lmemdoc)
	{
		LOGE("Failed to get memmory file");
		return 0;
	}
	memdoc = (unsigned char *)malloc(lmemdoc);
	memcpy(memdoc , buffer , lmemdoc);
	...
	fz_try(ctx)
	{
		...
		LOGE("Opening Memmory document...");
		fz_try(ctx)
		{
			ffile = fz_open_memory(ctx, memdoc, lmemdoc);
			if ( type == 'C') {
			  doc = (fz_document*) cbz_open_document_with_stream(ffile);
			}
			else if( type == 'X'){
			  doc = (fz_document*) xps_open_document_with_stream(ffile);
			} 
			else {
			  doc = (fz_document*) pdf_open_document_with_stream(ffile);
			}
		}
		fz_catch(ctx)
		{
			free(memdoc);
			lmemdoc = 0;
			fz_throw(ctx, "Cannot open memmory document\n");
		}
		LOGE("Done!");
		result = 1;
	}
...
Java_no_digipost_android_pdf_PDFCore_getMemDocInternal(JNIEnv *env, jobject thiz)
{
  unsigned char *tdoc = memdoc;
  long ltdoc = lmemdoc;
  ...
  if (!ltdoc)
  {
      if (stat((char*) memdoc , &fileinfo))
      {
		LOGE("Failed stat doc");
		return NULL;
      }
      int fd = open((char *)memdoc , O_RDONLY);
      tdoc = (unsigned char *) malloc(fileinfo.st_size);
      ltdoc = fileinfo.st_size;
      read(fd , tdoc , ltdoc);
      close(fd);
  }
  ret = (*env)->NewByteArray(env, ltdoc);
  (*env)->SetByteArrayRegion(env , ret , 0 ,ltdoc ,tdoc);
  if (!lmemdoc) free(tdoc);
  return ret;
}

JNIEXPORT int JNICALL
Java_no_digipost_android_pdf_PDFCore_countPagesInternal(JNIEnv *env, jobject thiz)
{
	return fz_count_pages(doc);
}

JNIEXPORT void JNICALL
Java_no_digipost_android_pdf_PDFCore_gotoPageInternal(JNIEnv *env, jobject thiz, int page)
{
	float zoom;
	fz_matrix ctm;
	fz_bbox bbox;
...
Java_no_digipost_android_pdf_PDFCore_getPageWidth(JNIEnv *env, jobject thiz)
{
...
JNIEXPORT float JNICALL
Java_no_digipost_android_pdf_PDFCore_getPageHeight(JNIEnv *env, jobject thiz)
{
...
Java_no_digipost_android_pdf_PDFCore_drawPage(JNIEnv *env, jobject thiz, jobject bitmap,
...
JNIEXPORT jboolean JNICALL
Java_no_digipost_android_pdf_PDFCore_needsPasswordInternal(JNIEnv * env, jobject thiz)
{
	return fz_needs_password(doc) ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_no_digipost_android_pdf_PDFCore_authenticatePasswordInternal(JNIEnv *env, jobject thiz, jstring password)
{
	const char *pw;
	int         result;
...
JNIEXPORT jboolean JNICALL
Java_no_digipost_android_pdf_PDFCore_hasOutlineInternal(JNIEnv * env, jobject thiz)
{
...
Java_no_digipost_android_pdf_PDFCore_getOutlineInternal(JNIEnv * env, jobject thiz)
{
	jclass        olClass;
	jmethodID     ctor;
	jobjectArray  arr;
	...
	olClass = (*env)->FindClass(env, "no/digipost/android/pdf/OutlineItem");
...
JNIEXPORT jobjectArray JNICALL
Java_no_digipost_android_pdf_PDFCore_searchPage(JNIEnv * env, jobject thiz, jstring jtext)
{
	jclass         rectClass;
	jmethodID      ctor;
	jobjectArray   arr;
...
JNIEXPORT void JNICALL
Java_no_digipost_android_pdf_PDFCore_destroying(JNIEnv * env, jobject thiz)
{
	...
	fz_close_document(doc);
	if (lmemdoc) free(memdoc);
	doc = NULL;
}

JNIEXPORT jobjectArray JNICALL
Java_no_digipost_android_pdf_PDFCore_getPageLinksInternal(JNIEnv * env, jobject thiz, int pageNumber)
{
	...
	linkInfoClass = (*env)->FindClass(env, "no/digipost/android/pdf/LinkInfo");
	if (linkInfoClass == NULL) return NULL;
	ctor = (*env)->GetMethodID(env, linkInfoClass, "<init>", "(FFFFI)V");
	if (ctor == NULL) return NULL;

	Java_no_digipost_android_pdf_PDFCore_gotoPageInternal(env, thiz, pageNumber);
...
JNIEXPORT int JNICALL
Java_no_digipost_android_pdf_PDFCore_getPageLink(JNIEnv * env, jobject thiz, int pageNumber, float x, float y)
{
	fz_matrix ctm;
	float zoom;
	fz_link *link;
